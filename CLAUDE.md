...
- **MaJ en masse sur les listes d'écarts MVOLA/OM (`/ecarts/mvola/ecarts/`, `/ecarts/om/ecarts/`) : sélection multi-lignes (case à cocher + tout sélectionner) + application groupée de 3 actions, demande explicite du 2026-09-15.** Actions retenues (les 2 autres possibles - pièce jointe, confirmation de rollback - explicitement exclues par l'utilisateur, trop spécifiques à une transaction pour avoir un sens en masse) :
  ...
  - **Garde-fou UX ajouté le 2026-09-15 : la sélection multiple et la barre d'action en masse ne s'affichent que si au moins un filtre est actif** (`filtre_applique`, calculé dans `_filtrer_ecarts(_om)` via `any(form.cleaned_data.values())` une fois le formulaire validé, propagé au contexte des deux vues `mvola_liste`/`om_liste`) — intention explicite de l'utilisateur : forcer à réfléchir/cibler avant une action de masse plutôt que de pouvoir agir en un clic sur la totalité des écarts non filtrés. Sans filtre, le tableau s'affiche normalement (sans colonne case à cocher) avec un message invitant à filtrer. C'est une nudge d'interface uniquement, pas un contrôle de sécurité : `mvola_bulk_action`/`om_bulk_action` continuent d'accepter n'importe quelle liste de `pk` côté serveur (seul le privilège `traiter_ecarts` est verifié), qu'un filtre ait ete applique ou non.
